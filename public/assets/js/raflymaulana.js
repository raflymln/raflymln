...
const setProjects = () => {
    $.post('/projects', (projects) => {
        projects.map((xi, page) => {
            page += 1;
            $('#pagination, #pagination2').append(`
            ...
            $('#project-list').append(`
                <section id="project-list-${page}" x-show="tab === 'page${page}'" class="relative container max-w-screen-xl mx-auto px-6 xl:px-0 pt-12 pb-24 mt-4 flex flex-col space-y-24"></section>
            `);

            xi.map(x => {
                var info;
                if (!x.moreinfo) {
                    info = `onclick="alert('No Info Available')"`
                } else {
                    info = `href="${x.moreinfo}"`
                }
                $(`#project-list-${page}`).append(`
                    <div class="flex flex-col-reverse lg:flex-row">
                        <section id="left" class="flex flex-col items-start justify-between flex-grow mt-4 pt-8 lg:pt-0 lg:mt-0 space-y-4">
                            ...
                            <section class="flex flex-row items-start space-x-3 w-full">
                                <a ${info} class="bg-yellow-300 btn w-full sm:w-max text-center sm:text-left border-b-4 rounded-3xl py-3 px-8 font-rubik font-bold text-white border-yellow-400">
                                    Learn More
                                </a>
                        ...
                        <section id="right" class="relative" style="background-image: url(${x.image});">
                            ...
                            <div class="w-full h-full relative z-20 flex flex-col justify-between items-center py-8">
                                ...
                                <section class="text-center">
                                    ...
                                    <h1 class="font-rubik font-bold text-xl mt-2 text-${x.status.color}-300">${x.status.name}</h1>
                                </section>
                            </div>
                        </section>
                    </div>
